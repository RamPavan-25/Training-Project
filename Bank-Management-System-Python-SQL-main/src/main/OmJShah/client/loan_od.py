...
def cp5(cur,acc_type,acc_no):
    ...
    if acc_type=="current":
        ...
        loan_or_od="loan"
    cur.execute(f"select {loan_or_od} from {acc_type} where acc_no={acc_no}")
    a=cur.fetchall()
    if a[0][0]=="NO" and acc_type=="savings":
        ...
        cur.execute("select %s_amt from %s where acc_no=%s",(loan_or_od,loan_or_od,acc_no))
        od=cur.fetchall()
        od=od[0][0]
        print("Your remaining od amount is ",od)
    else:
        print("You already have a loan pending to repay...")
        cur.execute("select %s_amt,%s_type from %s where acc_no=%s",(loan_or_od,loan_or_od,loan_or_od,acc_no))
        loan=cur.fetchall()
        loan_type=loan[0][1]
        ...
        loan_amt=loan[0][0]
        print("Your remaining od amount is %s of loan type %s",(loan_amt,loan_type))
